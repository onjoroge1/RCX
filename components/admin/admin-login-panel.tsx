...
import { adminSignInAction, type AuthFormState } from '@/lib/actions/auth'
import { PLATFORM_ADMIN_USERNAME } from '@/lib/admin/login-contract'
import { Logo } from '@/components/shared/logo'
import { Button } from '@/components/ui/button'
import { Field, Input, Label } from '@/components/ui/input'

export function AdminLoginPanel() {
  const [state, formAction, pending] = useActionState<AuthFormState, FormData>(adminSignInAction, undefined)
    ...
    <main className="flex min-h-screen items-center justify-center bg-background px-6 py-12">
      <div className="w-full max-w-md rounded-2xl border border-border bg-card p-8 rcx-shadow-lg">
        ...
        <p className="mt-2 text-sm leading-6 text-muted-foreground">
          Sign in with the dedicated platform administrator credentials. Customer workspace roles do not grant access to this control plane.
        </p>
        ...
        <form action={formAction} className="mt-6 flex flex-col gap-4">
          <Field>
            ...
            <Input
              ...
              defaultValue={PLATFORM_ADMIN_USERNAME}
              spellCheck={false}
            />
          </Field>
          <Field>
